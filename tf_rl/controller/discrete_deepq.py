...
class DiscreteDeepQ(object):
    def __init__(self, observation_shape,
        ...
        self.s = tf.Session(config=tf.ConfigProto(allow_soft_placement=True))

        self.s.run(tf.global_variables_initializer())
        self.s.run(self.target_network_update)

        self.saver = tf.train.Saver()
        self.s.graph.finalize()
    ...
    def save(self, save_dir, debug=False, global_step_val=0):
        ...
        self.saver.save(self.s, MODEL_FILE, global_step=global_step_val)
        with open(STATE_FILE, "wb") as f:
            pickle.dump(state, f)

        print('done in {} s'.format(time.time() - saving_started))

    def restore(self, save_dir, debug=False, global_step_val=0):
        # deepq state
        STATE_FILE      = os.path.join(save_dir, 'deepq_state')
        MODEL_FILE      = os.path.join(save_dir, 'model-'+str(global_step_val)+'.meta')

        with open(STATE_FILE, "rb") as f:
            state = pickle.load(f)
        self.saver = tf.train.import_meta_graph(MODEL_FILE)
        self.saver.restore(self.s, tf.train.latest_checkpoint(save_dir))
